...
# We show the monthly information as a structural form. We omit the index information.
df.index.freq = "MS"

# Let's divide for the parameters to be set
train = df[:120]
test = df[120:]
...
test_series = test['total_passengers']
...
arima_model = ARIMA(train, order=best_params_aic)
arima_model_fit = arima_model.fit()
y_pred = arima_model_fit.forecast(steps=24)[0]

y_pred_series = pd.Series(y_pred)
test_series = test_series[:len(y_pred_series)]
y_pred_series = y_pred_series[:len(test_series)]

mae = mean_absolute_error(test_series, y_pred_series)
print('MAE:', mae)
